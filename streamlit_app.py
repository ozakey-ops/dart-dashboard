...
def _render_ev_ebitda_chart(corp_code: str, yf_data: dict) -> None:
    ...
    _section_header(
        "EV/EBITDA 추이",
        "EV = 시가총액 + 부채총계 − 기말현금 · EBITDA = 영업이익 + 감가상각비(D) + 무형자산상각비(A)",
    )
    ...
    da_missing_years: list[str] = []          # D&A 없어서 영업이익만 쓴 연도
    ...
    for yr_str, mc in mktcap.items():
        ...
        dep = fd["cf"].get("depreciationDA") or 0   # 유형자산 감가상각비
        amt = fd["cf"].get("amortizationIA") or 0   # 무형자산 상각비
        # 현금흐름표 조정항목은 절댓값으로 기재되므로 항상 양수
        dep = abs(dep)
        amt = abs(amt)
        ebitda = op_income + dep + amt               # 단위: 억원
...
# ──────────────────────────────────────────
# 적정 주가 산출 방법 추천 조합 (참고)
#
# 기업 유형          주력 방식    교차검증
# ─────────────────────────────────────────
# 성장주 (IT·바이오)  DCF         PER
# 가치주 (제조·소비재) PER        PBR
# 금융주 (은행·보험)  PBR         ROE-PBR
# 적자 기업          PBR          EV/EBITDA
#
# 아래 함수는 PER / PBR / DCF 3중 방식으로
# 각 적정 주가와 현재가 대비 괴리율을 계산합니다.
# ──────────────────────────────────────────
def _render_fair_value_card(corp_code: str, yf_data: dict,
                             stock_code: str, corp_cls: str) -> None:
    """PER / PBR / DCF 3중 적정 주가 요약 카드."""
    ...
    if ocf_vals:
        ...
        tv     = fcf * (1 + gt) / (wacc - gt)
        pv_tv  = tv / (1 + wacc) ** 5
        intrinsic = (pv_sum + pv_tv) * 1e8   # 원 단위
        dcf_fair  = round(intrinsic / shares)
    ...
    def _card(title: str, fair: int | None, gap_str: str, gap_clr: str,
              sub: str = "") -> str:
    ...
    cards_html = (
        ...
        + _card("DCF 적정가",  dcf_fair,  dcf_gap_str,  dcf_gap_clr,
                "g=8% / TV=2% / r=10%")
        + _card("종합 적정가", consensus, cons_gap_str, cons_gap_clr,
                "3방식 평균")
    )
...
def main() -> None:
    ...
    # 적정 주가 종합 분석 카드 (환율 위)
    stock_code_main = corp.get("stock_code", "")
    corp_code_main  = corp.get("corp_code", "")
    if stock_code_main and corp_code_main:
        _yf = fetch_yf_annual_data(
            stock_code_main, ov.get("corp_cls_raw", "Y"),
            corp_code_main, _ver=_CACHE_VER,
        )
        if "__error__" not in _yf:
            _render_fair_value_card(
                corp_code_main, _yf, stock_code_main, ov.get("corp_cls_raw", "Y"),
            )
