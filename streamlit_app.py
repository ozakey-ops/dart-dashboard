...
def render_stock_chart(stock_code, corp_name, corp_cls="Y", corp_code=None):
    ...
    if corp_code:
        ...
        with st.spinner("직원 현황 조회 중..."):
            emp_data = fetch_employee_status(corp_code, cache_ver=_CACHE_VER)

        _section_header("정규직 현황")
        if emp_data:
            eyears = sorted(emp_data.keys())

            # 최신연도 요약 카드
            latest_y = eyears[-1]
            ed = emp_data[latest_y]
            total   = ed.get("total", 0)
            male    = ed.get("male", 0)
            female  = ed.get("female", 0)
            sal_m   = ed.get("salary_m")
            sal_f   = ed.get("salary_f")
            ten_m   = ed.get("avg_tenure_m", 0)
            ten_f   = ed.get("avg_tenure_f", 0)

            def _kpi(label, val, sub=""):
                sub_html = f'<div style="font-size:.65rem;color:#94a3b8;margin-top:1px">{sub}</div>' if sub else ""
                return (f'<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;'
                        f'padding:8px 10px;text-align:center;">'
                        f'<div style="font-size:.65rem;color:#64748b;margin-bottom:2px">{label}</div>'
                        f'<div style="font-size:1rem;font-weight:700;color:#1e293b">{val}</div>'
                        f'{sub_html}</div>')

            sal_m_str = f'{round((sal_m or 0)/10_000):,}만원' if sal_m else "-"
            sal_f_str = f'{round((sal_f or 0)/10_000):,}만원' if sal_f else "-"

            st.markdown(
                f'<div style="font-size:.72rem;color:#94a3b8;margin-bottom:6px">'
                f'{latest_y}년 기준 사업보고서</div>'
                f'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px">'
                f'{_kpi("전체 정규직", f"{total:,}명")}'
                f'{_kpi("남 정규직", f"{male:,}명", f"평균근속 {ten_m}년 · 연봉 {sal_m_str}")}'
                f'{_kpi("여 정규직", f"{female:,}명", f"평균근속 {ten_f}년 · 연봉 {sal_f_str}")}'
                f'</div>',
                unsafe_allow_html=True,
            )
